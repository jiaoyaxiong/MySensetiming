...
class test_video_abnormal_006(common.sensemediaTestBase):
    ...
    def __init__(self):
        ...
        self.stream = "rtsp://172.20.14.63:8081/test.ts"
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Video extract frame error, pls check the video":
            self.logger.error("error_msg should be Video extract frame error, pls check the video ")
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010117:
            assert False
