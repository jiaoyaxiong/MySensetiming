...
"""
@attr(feature="test_video_modules_common")
@attr(runtype="normal")
@attr(videotype="normal")
class test_vm_normal_021(common.sensemediaTestBase):

    def __init__(self):
        super(test_vm_normal_021, self).__init__("test_vm_normal_021")
        ...
        self.file = ""
        # TODO
        self.video_url = "https://172.20.14.63:6554/shot_dplit.mp4"

        # self.video_url = "http://172.30.1.101/JiangSuHD_1533611640_1533612600.mp4"
    ...
    def check_result_urls(self,res_res_url,resp_id):
        ...
        # 特殊场景，需要判断id是否超范围
        start_frame_ids=[x.get("start_frame_id") for x in shot_results ]
        end_frame_ids=[x.get("end_frame_id") for x in shot_results ]

        self.logger.info("start_frame_ids is %s" % start_frame_ids)
        self.logger.info("end_frame_ids is  %s " % end_frame_ids)


        #判断shot end_id 不能大于success 最大id
        # id from 0
        max_shot_id = max(end_frame_ids)
        if max_shot_id > len(ids)-1:
            self.logger.error("max_shot_id is %s , success max id is %s ,not as expected " % (max_shot_id,len(ids)-1))
            assert False
