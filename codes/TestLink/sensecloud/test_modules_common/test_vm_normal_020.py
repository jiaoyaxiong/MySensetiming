...
class test_vm_normal_020(common.sensemediaTestBase):
    ...
    def __init__(self):
        ...
        self.file = ""
        # TODO
        self.video_url = "http://172.20.14.63:90/renmingdemingyi.mp4"
        self.stream = ""
        ...
        self.body = {
            ...
            "token": self.token,
            "modules": "filter_star:12,filter_star:500"
        }
        # 请求头
    ...
    def test_001(self):
        ...
        if r.json().get("error_msg") != "Module is repeat: module is filter_star":
            self.logger.error(
                "should be Module is repeat: module is filter_star but is  %s " % r.json().get("error_msg"))
            assert False

        if r.json().get("http_code") != 400:
            assert False

        if r.json().get("error_code") != 1010119:
            assert False
