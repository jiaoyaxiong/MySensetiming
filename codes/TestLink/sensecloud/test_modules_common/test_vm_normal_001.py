...
class test_vm_normal_001(common.sensemediaTestBase):
    ...
    def __init__(self):
        ...
        self.body = {
            ...
            "token": self.token,
            "modules": "filter_star,filter_politician,filter_terror,filter_np,filter_celebrity_attr,ocr,cloth_detect,object_detect,scene_detect,event_detect"
        }
        # 请求头
    ...
    def check_result_urls(self,res_res_url,resp_id):
        ...
        if len(modules) != 10:
            self.logger.error("there should 10 modules ! but is : %s " % len(modules))
            assert False
