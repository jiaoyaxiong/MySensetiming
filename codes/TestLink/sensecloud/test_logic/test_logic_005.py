...
class test_logic_005(common.sensemediaTestBase):

    def __init__(self):
        super(test_logic_005, self).__init__("test_logic_005")
    ...
    def test_001(self):
        ...
        self.logger.info("---step16  朋友圈美女图片两张　入库　　　李欣萌  明星　")

        url_16 = self.baseurl + "/sensemedia/face_in/v2/persons/?db_name=明星"
        self.file = ("twoman.zip", open('/codes/sensecloud/test_logic/twoman.zip', 'rb'), "application/zip")
        # 请求体
        ...
        for x in r_17.json().get("results"):
            if x["imageName"].encode("utf-8") == "李欣萌_绝色美女/lixingmeng1.png":
                id = x["id"]
        ...
        data_21_2 = {"part": "朋友圈美女", "persons": ["天子娇女"]}
        r_21 = requests.post(url_21, data=json.dumps(data_21_2), headers=self.json_headers)
        self.logger.info(r_21.url)
        self.logger.info(r_21.text)
        ...
        self.logger.info("---step29  删除　朋友圈美女　子库")
        data_27 = {"name": "朋友圈美女"}
